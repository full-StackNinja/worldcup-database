...
echo "$($PSQL "SELECT year, name FROM games LEFT JOIN teams ON games.winner_id = teams.team_id  WHERE round = 'Final' ORDER BY year DESC")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "SELECT DISTINCT(name) FROM games LEFT JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE name LIKE 'Co%' ORDER BY name")"
